...
function SrsPlayer(container, width, height, private_object) {
    ...
    var __HTML = '<div style="background:#424242;text-align: center;"><a href="http://www.adobe.com/go/getflashplayer" target="_blank" title="升级Flash插件" style="height:40px;line-height:40px;text-align:center;color:#fff;overflow:hidden;text-overflow:ellipsis;margin:auto;text-decoration:none;">点击升级 Flash 插件</a></div>';
    var elem = document.getElementById(container);
    if (!hasFlash || !versionOK) {
        elem.innerHTML = __HTML;
        elem.children[0].style.width = width + 'px';
        elem.children[0].style.height = height + 'px';
        elem.children[0].style.lineHeight = height + 'px';
        return;
    }
    ...
    this.srs_player_url = "srs_player/bin-release/SrsPlayer.swf?_version=" + (+new Date());
...
SrsPlayer.prototype.start = function(url) {
    ...
    swfobject.embedSWF(
        ...
        "11.1.0", "//s.ssl.qhres.com/static/db3e04bd6db0bbba.swf",
 ...
 */
 SrsPlayer.prototype.set_srs_player_url = function(url, params) {
    var query_array = [], 
        ...
        p;
    params = params || {}; 
    params._version = this.__VERSION;
    for (p in params) {
